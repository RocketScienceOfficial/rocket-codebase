from sim.env.environments import *
from sim.env.physics_engines import *
from sim.utils.geo import g


class Scenario(SimpleIntegratorScenarioInterface):
    def __init__(self, max_time: float | None = None):
        self.time = 0.0
        self.max_time = max_time if max_time is not None else 10.0

    def get_net_acc(self, time: float, current_state: PhysicsEngineOutput, input: PhysicsEngineInput) -> np.ndarray:
        self.time = time

        if time < 3.0:
            return np.array([0.0, 0.0, -4.5 * g])
        else:
            return np.array([0.0, 0.0, g])

    def finished(self) -> bool:
        return self.time >= self.max_time


def get_environment(dt: float):
    return SyntheticEnvironment(
        engine=SimpleIntegratorPhysicsEngine(dt=dt, scenario=Scenario(max_time=25.0), init_time=2.0),
        imu1=SyntheticIMUModel(rate=500, noise_acc=GaussianNoiseModel(mean=0, std_dev=0.3), noise_gyro=GaussianNoiseModel(mean=0, std_dev=0.17), acc_range_g=6.0, gyro_range_deg=500.0),
        mag1=SyntheticMagnetometerModel(rate=100, noise=GaussianNoiseModel(mean=0, std_dev=0.0004)),
        baro1=SyntheticBarometerModel(rate=25, noise=GaussianNoiseModel(mean=0, std_dev=3.0)),
        gps1=SyntheticGPSModel(rate=25, noise_hor=GaussianNoiseModel(mean=0, std_dev=1.7), noise_ver=GaussianNoiseModel(mean=0, std_dev=3.1), noise_vel=GaussianNoiseModel(mean=0, std_dev=0.5), lat=50.337497, lon=19.525838, alt=30),
    )
