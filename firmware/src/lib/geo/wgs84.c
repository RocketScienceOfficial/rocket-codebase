...
static geo_position_t _ecef_to_geo(const vec3_prec_t *pos)
{
    ...
    double miu = atan((pos->z + EARTH_ECCENTRICITY_SQUARED * (1.0 - EARTH_FLATTENING) / (1.0 - EARTH_ECCENTRICITY_SQUARED) * EARTH_SEMI_MAJOR_AXIS * sin_beta * sin_beta * sin_beta) / (s - EARTH_ECCENTRICITY_SQUARED * EARTH_SEMI_MAJOR_AXIS * cos_beta * cos_beta * cos_beta));
    double err = 1e10;
    int iter = 0;
    ...
    while (err > 1e-10 && iter < MAX_ITER)
    {
        ...
        sin_beta = sin(beta);
        cos_beta = cos(beta);
        miu = atan((pos->z + EARTH_ECCENTRICITY_SQUARED * (1.0 - EARTH_FLATTENING) / (1.0 - EARTH_ECCENTRICITY_SQUARED) * EARTH_SEMI_MAJOR_AXIS * sin_beta * sin_beta * sin_beta) / (s - EARTH_ECCENTRICITY_SQUARED * EARTH_SEMI_MAJOR_AXIS * cos_beta * cos_beta * cos_beta));
