...
static void __not_in_flash_func(flash_write_helper)(void *arg)
{
...
void hal_flash_write_pages(size_t offsetPages, const uint8_t *buffer, size_t pagesCount)
{
    ...
    flash_safe_execute(flash_write_helper, &args, 1000);

    busy_wait_us(10);
}

static void __not_in_flash_func(flash_erase_helper)(void *arg)
{
    flash_args_t *args = (flash_args_t *)arg;
...
void hal_flash_erase_sectors(size_t sectorsOffset, size_t sectorsCount)
{
    ...
    flash_safe_execute(flash_erase_helper, &args, 1000);
