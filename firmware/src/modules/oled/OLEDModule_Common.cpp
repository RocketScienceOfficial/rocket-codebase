...
static void draw_left_str_f(u8g2_t *u8g2, uint8_t x, uint8_t y, const char *str, ...)
{
    static char buf[128];
...
static void draw_centered_str_f(u8g2_t *u8g2, uint8_t x, uint8_t y, const char *str, ...)
{
    static char buf[128];
...
static void draw_right_str_f(u8g2_t *u8g2, uint8_t x, uint8_t y, const char *str, ...)
{
    static char buf[128];
...
static float get_rssi_percentage(float rssi)
{
...
static void draw_progress_bar(u8g2_t *u8g2, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t progress)
{
    u8g2_DrawBox(u8g2, x, y, width * (float)progress / 100.0f, height);
}

static void draw_battery_indicator(u8g2_t *u8g2, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t percentage)
{
...
void OLEDModule::drawPanel()
{
    ...
    {
        draw_progress_bar(&m_Display, 0, 2, 45, 6, (uint8_t)get_rssi_percentage(m_CurrentData->rssi));
        draw_left_str_f(&m_Display, 52, 10, "%d", m_CurrentData->rssi);
    }
    else
    {
        draw_left_str_f(&m_Display, 0, 10, "NO SIGNAL");
    }

    draw_right_str_f(&m_Display, 125, 10, "%s", m_CurrentData->name);

    draw_left_str_f(&m_Display, 0, 23, "RX: %d", m_CurrentData->rx);
    draw_left_str_f(&m_Display, 50, 23, "TX: %d", m_CurrentData->tx);
    draw_left_str_f(&m_Display, 0, 36, "EXEC: %ds", m_CurrentData->execTimeoutLeft);
    draw_left_str_f(&m_Display, 0, 50, "%.7f", m_CurrentData->lat);
    draw_left_str_f(&m_Display, 0, 62, "%.7f", m_CurrentData->lon);

    draw_right_str_f(&m_Display, 125, 28, "%d %%", m_CurrentData->batteryPercentage);
    draw_battery_indicator(&m_Display, 110, 41, 30, 15, m_CurrentData->batteryPercentage);
    draw_right_str_f(&m_Display, 125, 63, "%.2f V", m_CurrentData->batteryVoltage);
}