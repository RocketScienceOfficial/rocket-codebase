...
static uint8_t bmi088_acc_read_reg(const bmi088_acc_device_t *device, uint8_t address)
{
    uint8_t data[2];
...
static void bmi088_acc_read_regs(const bmi088_acc_device_t *device, uint8_t address, uint8_t *buffer, size_t count)
{
...
static void bmi088_acc_write_reg(const bmi088_acc_device_t *device, uint8_t address, uint8_t data)
{
    bus_utils_write_reg(&device->device, address, data);
}

static void bmi088_acc_set_power(const bmi088_acc_device_t *device, bool on)
{
    uint8_t data = on ? BMI088_ACC_PWR_CTRL_ON : BMI088_ACC_PWR_CTRL_OFF;

    bmi088_acc_write_reg(device, BMI088_ACC_REG_PWR_CTRL, data);

    time_utils_delay_us_osal(BMI088_ACC_PWR_CTRL_TIMEOUT_US);
}

static void bmi088_acc_set_mode(const bmi088_acc_device_t *device, bool active)
{
    uint8_t data = active ? BMI088_ACC_PWR_CONF_ACTIVE : BMI088_ACC_PWR_CONF_SUSPEND;

    bmi088_acc_write_reg(device, BMI088_ACC_REG_PWR_CONF, data);

    time_utils_delay_us_osal(BMI088_ACC_PWR_CONF_TIMEOUT_US);
}

static void bmi088_acc_init_base(bmi088_acc_device_t *device)
{
    bmi088_acc_set_power(device, true);
...
bool bmi088_acc_validate(const bmi088_acc_device_t *device)
{
    ...
    return bmi088_acc_read_reg(device, BMI088_ACC_REG_CHIP_ID) == BMI088_ACC_CHIP_ID;
}
...
void bmi088_acc_set_conf(const bmi088_acc_device_t *device, bmi088_acc_odr_t odr, bmi088_acc_osr_t osr)
{
    ...
    bmi088_acc_write_reg(device, BMI088_ACC_REG_CONF, (uint8_t)odr | ((uint8_t)osr << BMI088_ACC_CONF_OSR_BITSHIFT));
...
void bmi088_acc_set_range(bmi088_acc_device_t *device, bmi088_acc_range_t range)
{
    ...
    bmi088_acc_write_reg(device, BMI088_ACC_REG_RANGE, (uint8_t)range);
}
...
void bmi088_acc_read(const bmi088_acc_device_t *device, vec3_t *accel)
{
    ...
    bmi088_acc_read_regs(device, BMI088_ACC_REG_DATA, buff, 6);
